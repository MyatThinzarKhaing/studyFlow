...
export function HomeContent(){ 
  ...
  // Load saved filename from localStorage when the home page mounts
  useEffect(() => {
    const savedName = localStorage.getItem('active_pdf_filename');
    if (savedName) {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.35fr_1fr]">
        <button onClick={() => input.current?.click()} className="group flex min-h-64 flex-col items-center justify-center rounded-3xl border-2 border-dashed border-border bg-card px-6 text-center transition hover:border-primary/50 hover:bg-muted">
          ...
          <h2 className="mt-5 text-lg font-semibold">{busy ? 'Uploading PDF…' : uploaded ? fileName : 'Drop your PDF here'}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{uploaded ? 'PDF uploaded successfully' : 'or click to browse from your device'}</p>
        ...
        <div className="flex flex-col justify-between rounded-3xl bg-primary p-7 text-primary-foreground">
          <div>
            ...
            <h2 className="mt-5 text-2xl font-semibold tracking-tight truncate max-w-[280px]" title={fileName}>{fileName || 'Introduction to Psychology'}</h2>
            <p className="mt-2 text-sm text-primary-foreground/70">Ready for study tools</p>
          </div>
          <div className="mt-8">
            <div className="flex justify-between text-xs text-primary-foreground/70"><span>Status</span><span>{uploaded ? 'Uploaded' : 'Ready'}</span></div>
            <div className="mt-2 h-2 rounded-full bg-primary-foreground/15"><div className="h-full w-full rounded-full bg-accent"/></div>
            <Link href="/flashcards" className="mt-6 flex items-center gap-2 text-sm font-semibold">Go to Flashcards <ArrowRight className="size-4"/></Link>
          </div>
        </div>
      </div>
      <section>
